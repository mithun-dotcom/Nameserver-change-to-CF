...
async function setSpaceship({ domain, ns, creds }) {
  ...
  const res = await fetch(`https://spaceship.dev/api/v1/domains/${domain}/nameservers`, {
    ...
    // Spaceship expects: { provider: "custom", hosts: ["ns1...", "ns2..."] }
    body: JSON.stringify({ provider: "custom", hosts: ns }),
  });
  if (res.ok) return "NS set at registrar";

  // Surface Spaceship's real error detail instead of a bare HTTP code
  const j = await res.json().catch(() => ({}));
  const detail =
    j.detail ||
    j.message ||
    (Array.isArray(j.errors) && j.errors.map((e) => e.message || e.detail).filter(Boolean).join("; ")) ||
    `HTTP ${res.status}`;
